...
export class InvestmentSummaryPage extends BasePage {
  ...
  private readonly stripeFrame = this.page.frameLocator(
    "#payment-element iframe",
  );
  private readonly cardNumberField: Locator = this.stripeFrame.getByPlaceholder(
    "1234 1234 1234 1234",
  );
  private readonly expiryDateField =
    this.stripeFrame.getByPlaceholder("MM / YY");
  private readonly securityCodeField = this.stripeFrame.getByPlaceholder("CVC");
  private readonly continueWithPaymentButton = this.page.getByRole("button", {
    name: "Continue with Payment",
  });
  private readonly countryDropdown = this.stripeFrame.locator(
  ...
  async selectIndiaCountryFromDropdown(): Promise<void> {
    logger.info(this.pageName, "Selecting 'India' from country dropdown");
