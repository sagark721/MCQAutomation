...
setup.describe("Authentication Setup", () => {
  setup(
    "Authenticate as Canadian Individual i.e CAN_individual user @smoke",
    async ({ page }) => {
      if (
        !process.env.CAN_INDIVIDUAL_EMAIL ||
        !process.env.CAN_INDIVIDUAL_PASSWORD
      ) {
        throw new Error(
          "Missing credentials for CAN_individual user. Please set CAN_INDIVIDUAL_EMAIL and CAN_INDIVIDUAL_PASSWORD in your environment variables.",
        );
      }
